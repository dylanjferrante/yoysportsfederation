...
// Weekly schedule / matchup editor — embedded as a tab inside commissioner settings.
export default function ScheduleEditor({ leagueId }: { leagueId: string }) {
  ...
  const load = () => fetch(`/api/leagues/${leagueId}`).then(r => r.json()).then(d => {
    setLeague(d.league)
  ...
  async function patch(payload: any) {
    setBusy(true)
    await fetch(`/api/leagues/${leagueId}/matchups`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
    setBusy(false); await load()
  }
  const setM = (id: string, p: Partial<Matchup>) => setMatchups(prev => prev.map(m => m.id === id ? { ...m, ...p } : m))

  if (!league) return <div className="text-center py-12 text-slate-400">Loading schedule…</div>

  return (
    <>
      <div>
        <h3 className="font-semibold text-slate-900">Schedule &amp; Matchups</h3>
        <p className="text-sm text-slate-500">Edit weekly pairings and override results across all sports.</p>
      </div>
