  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="rounded-2xl p-5 mt-3 mb-6 flex items-center gap-4 flex-wrap" style={{ background: primary || '#0f172a', color: secondary || '#ffffff' }}>
        {logo
          ? <img src={logo} alt="" className="w-16 h-16 object-contain bg-white/10 p-1.5" />
      ...
      {recs.length > 0 && (
        <div className="flex gap-2 flex-wrap mb-5">
          {recs.map(r => (
            <span key={r.id} className="text-xs px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 font-medium">{r.sport}: {r.wins}-{r.losses}{r.ties ? `-${r.ties}` : ''}{r.finishPosition ? ` · #${r.finishPosition}` : ''}</span>
          ))}
        </div>
      )}
